                ...
                <#if realm.password>
					<form id="kc-form-login"
						...
						<div class="${properties.kcFormGroupClass!} ${properties.kcFormSettingClass!}">
                            ...
                            <#if properties.showForgotZugangsdatenVideo=='true'>
								<div style="flex-basis: 100%; height: 10px; margin: 0; border: 0;"></div>
								<div class="${properties.kcFormOptionsWrapperClass!}" style="margin-left: auto">
                                    <#if realm.name == 'vacme' >
				...
				<div id="kc-social-providers"
					...
					<ul class="${properties.kcFormSocialAccountListClass!} <#if social.providers?size gt 4>${properties.kcFormSocialAccountDoubleListClass!}</#if>">
                        <#list social.providers as p>
                            <#if p.alias == 'hin-idp'>
                            <#-- HIN button has a special styling  -->
                                <#if properties.showHinLoginButton=="true">
                                <#-- HIN Button: show only if enabled in theme.properties, otherwise hide -->
									<li>
										<div class="hin-col">
											<p>
												<a href="${p.loginUrl}"
												   ...
												   class="btn hin-social hin-btn hin-login-with-hin">
													<i class="hin-icon"><img src="${url.resourcesPath}/img/hin_logo.png"></i>
													<span class="login">${p.displayName}</span></a>
											</p>
										</div>
									</li>
                                </#if>
    ...
    <#elseif section = "info" >
        <#if realm.password && realm.registrationAllowed && !registrationDisabled??>
			<div id="kc-registration">
                <#if properties.showSpecificRegisterLink=="true">
					<!-- TODO man koennte evtl fuer ZH auch /impffaehigkeit als registrationUrl hinterlegen? -->
					<span>${msg("noAccount")} <a tabindex="0" href="/impffaehigkeit">${msg("doRegister")}</a></span>
                <#else>
