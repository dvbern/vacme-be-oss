...
import {ControlValueAccessor, UntypedFormControl, FormGroupDirective, FormGroupName} from '@angular/forms';
...
export abstract class FormControlBase implements OnInit, AfterViewInit, ControlValueAccessor {
    ...
    public ngOnInit(): void {
        if (this.controlName && this.container) {
            this._control = this.container.control.controls[this.controlName] as UntypedFormControl;
        }
    }
...
function findControlName(control: UntypedFormControl | Empty): string | undefined {
    if (!control) {
        return undefined;
    }
