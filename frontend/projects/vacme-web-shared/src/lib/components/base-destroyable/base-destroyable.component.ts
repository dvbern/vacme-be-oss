...
export abstract class BaseDestroyableComponent implements OnDestroy {
    ...
    private readonly _destroyedEmitter$ = new Subject<void>();


    ngOnDestroy(): void {
        this._destroyedEmitter$.next();
        this._destroyedEmitter$.complete();
    }

    public get destroyedEmitter$(): Observable<void> {
        return this._destroyedEmitter$.asObservable();
    }
    ...
    protected takeUntilDestroyed<T>(): MonoTypeOperatorFunction<T> {
        return takeUntil(this._destroyedEmitter$);
    }
