...
export class NavigationService {
    ...
    public navigate(dossiersOverview: ImpfdossiersOverviewJaxTS): void {
        ...
        const krankheit = this.authServiceRS.getKrankheitForUserIfOnlyAllowedForExactlyOne();
        if (KrankheitIdentifierTS.COVID !== krankheit) {
            this.toGeimpft(dossiersOverview.registrierungsnummer);
            return;
        }
        // Wenn wir hier hin kommen, ist die (einzige) Krankheit COVID
        this.navigateForCovid(dossiersOverview, krankheit);
    }

    private navigateForCovid(impfdossiersOverviewJaxTS: ImpfdossiersOverviewJaxTS, krankheit: KrankheitIdentifierTS.COVID): void {
        ...
        switch (dossier.status) {
            ...
            case ImpfdossierStatusTS.REGISTRIERT:
