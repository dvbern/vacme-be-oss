...
export class KontrolleResolverService implements Resolve<any> {
    ...
    public resolve(route: ActivatedRouteSnapshot): Observable<any> {
        ...
        if (registrierungsnummer !== undefined && registrierungsnummer !== 'new' && krankheitFromRoute) {
            const fileInfoRequest$ = this.dossierService.dossierResourceGetFileInfo(krankheitFromRoute, registrierungsnummer);
            const dashboardRequest$ = this.dossierService.dossierResourceGetDashboardImpfdossier(krankheitFromRoute, registrierungsnummer);
            return forkJoin({
