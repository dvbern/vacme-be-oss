...
export const resolveOrtDerImpfung: ResolveFn<Observable<OrtDerImpfungJaxTS | undefined>> =
    (route: ActivatedRouteSnapshot, state: RouterStateSnapshot) => {
        const ortderimpfungService = inject(OrtderimpfungService);
        const id = route.params.ortDerImpfungId;
        if (id) {
            if (id) {
                return ortderimpfungService.ortDerImpfungResourceGetOrtDerImpfung(id);
            }
        }
        return of(undefined);
    };
