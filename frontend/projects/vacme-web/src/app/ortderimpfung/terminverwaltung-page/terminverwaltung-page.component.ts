...
export class TerminverwaltungPageComponent extends BaseDestroyableComponent implements OnInit, CanComponentDeactivate {
    ...
    private createTableRow(): FormGroup[] {
        ...
        for (const row of this.rows) {
            ...
            for (const krankheit of this.verfuegbareKrankheiten) {
                (rowGroup.get('krankheitImpfslotDisplayJaxMap') as FormGroup).addControl(krankheit,
    ...
    private createRowCell(row: ImpfslotDisplayDayJaxTS, krankheit: KrankheitIdentifierTS): FormGroup[] {
        const result = new Array();
    ...
    public getDayTimeslotFormgroup(dayFormGroup: FormGroup, col: number, krankheit: KrankheitIdentifierTS): FormGroup {
        const colArray = dayFormGroup.controls.krankheitImpfslotDisplayJaxMap.get(krankheit) as FormArray;
        return colArray.controls[col] as FormGroup;
    }

    public getControl(row: number, col: number, controlName: string, krankheit: KrankheitIdentifierTS): FormControl {
        return this.getDayTimeslotFormgroup(this.getDayFormgroup(row), col, krankheit).controls[controlName] as FormControl;
    }
    ...
    public getSum(rowIndex: number, impffolge: ImpffolgeTS, krankheit: KrankheitIdentifierTS): number | undefined {
        ...
        const dayKrankheitFormgroupArray = dayFormGroup.controls.krankheitImpfslotDisplayJaxMap.get(krankheit) as FormArray;
