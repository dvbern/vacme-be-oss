...
import {ZertifikatService} from 'vacme-web-generated';
import {
    ...
    public termineUmbuchen(data: TSPersonFolgetermin): void {
        ...
        this.terminmanager.canProceedTermineUmbuchen$(this.isTerminbuchungAllowed(data))
            .subscribe(canProceed => {
                if (canProceed) {
                    this.router.navigate(['person', this.dashboardJax?.registrierungsnummer, 'geimpftterminbearbeitung', data.krankheit]);
                }
            }, error => {
                LOG.error(error);
            });
