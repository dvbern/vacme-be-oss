...
export class GeimpftPageComponent extends BaseDestroyableComponent implements OnInit {
    ...
    public registrierungJax?: RegistrierungBasicInfoJaxTS;
    ...
    private initFromActiveRoute(): void {
        this.activeRoute.data
            ...
            .subscribe(next => {
                if (next.data) {
                    if (next.data.registrierung$) {
                        this.registrierungJax = next.data.registrierung$;
                        this.accessOk = true;
                    }
    ...
    private loadZertifikatList(): void {
        if (this.dashboardCovidJax?.registrierungsnummer) {
            this.dossierService.dossierResourceGetAllZertifikate(this.getKrankheit(),
    ...
    title(): string {
        return this.translationService.instant('GEIMPFT.TITLE',
            {person: this.getPersonInfos(this.registrierungJax)});
    }

    private getPersonInfos(dashboardJax: RegistrierungBasicInfoJaxTS | undefined): string {
        if (!dashboardJax) {
            return '';
        }
    ...
    downloadImpfdokumentation(krankheit: KrankheitIdentifierTS): void {
        this.downloadService.downloadResourceDownloadImpfdokumentation(
            krankheit,
            this.registrierungJax?.registrierungsnummer as string).subscribe(BlobUtil.openInNewTab, _ => {
            LOG.error('Could not download Impfdokumentation for registration '
                + this.registrierungJax?.registrierungsnummer);
        });
    }
    ...
    public navigateToKontrolleForCovid(): void {
        // Neu ist nicht mehr sichergestellt, dass es fuer jede Reg ein Covid-Dossier gibt. Wir muessen also beim
        // Navigieren auf die Kontrolle eines erstellen, falls es noch nich da ist.
        const registrierungsnummer = this.registrierungJax?.registrierungsnummer;
        if (registrierungsnummer) {
            this.dossierService.dossierResourceCreateImpfdossier(KrankheitIdentifierTS.COVID, registrierungsnummer)
                .subscribe(() => {
