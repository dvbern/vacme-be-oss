...
export class ErkrankungPageComponent extends BaseDestroyableComponent implements OnInit, CanComponentDeactivateSimple {
    ...
    createErkrankungFormgroup(erkrankung: ErkrankungJaxTS, index: number): UntypedFormGroup {
     ...
     * fields
     */
    private addValueChangeListenersForDependentValidation(erkFormGrp: UntypedFormGroup): void {
    ...
    getErkrankungenFormArray(): UntypedFormArray | undefined {
        return this.formGroup.get('erkrankungen') as UntypedFormArray;
    }

    getErkrankungFormGroup(erkrankungFormGroup: any): UntypedFormGroup {
        return erkrankungFormGroup as UntypedFormGroup;
    }
    ...
    canDelete(erkrankungFormGroup: UntypedFormGroup): boolean {
    ...
    private addBasic(erkrankung: ErkrankungJaxTS): void {
        const erkrankungArray = this.formGroup.get('erkrankungen') as UntypedFormArray;
    ...
    private save(): void {
        ...
        this.dossierService.dossierResourceRegUpdateErkrankungen(this.getKrankheit(), load.regNr, true, load.data)
            .subscribe(
                dashboardReloadedTest => {
                    ...
                    canProceed$.subscribe(canProceed => {
                        if (canProceed) {
                            // Wirklich speichern
                            this.dossierService.dossierResourceRegUpdateErkrankungen(this.getKrankheit(), load.regNr,
    ...
    private doAfterSaved(
        ...
        _: { data: ErkrankungJaxTS[] | undefined; regNr: string },
    ): void {
    ...
    public getErkrankungHintKey(erkrankungFormGroup: UntypedFormGroup): string {
        return 'ERKRANKUNGEN.DATE_HINT.' + (erkrankungFormGroup.value as ErkrankungJaxTS).erkrankungdatumHerkunft;
    }

    public reValidateAllControlsExceptPassed(erkFormGrp: UntypedFormGroup): void {
