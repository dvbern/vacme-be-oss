...
export const resolveDossier: ResolveFn<Observable<DashboardJaxTS>> =
    (route: ActivatedRouteSnapshot, state: RouterStateSnapshot) => {
        ...
        if (registrierungsnummer && krankheitFromRoute) {
            if (registrierungsnummer) {
                ...
                return dossierService.dossierResourceRegGetDashboardRegistrierung(
                    krankheitFromRoute,
                    registrierungsnummer);
            }
        }
        throw Error('Missing route-param registrierungsnummer or krankheit');
    };
