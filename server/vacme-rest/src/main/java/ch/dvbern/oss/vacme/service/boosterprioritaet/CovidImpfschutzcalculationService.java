...
public class CovidImpfschutzcalculationService implements ImpfschutzCalculationService {
	...
	public Optional<Impfschutz> calculateImpfschutz(
		...
		if (isGrundimmunisiertGemaessVacMeOrSelbstdeklaration(impfinformationDto.getImpfdossier())) {
			...
			// Bei COVID behandelt die Engine erst die Status ab IMMUNISIERT
			// Fuer Grundimmunisierung muss es separat berechnet werden
			impfdossierService.setStatusAccordingToPrioritaetFreischaltung(impfinformationDto.getImpfdossier());
		}
		LOG.debug(
