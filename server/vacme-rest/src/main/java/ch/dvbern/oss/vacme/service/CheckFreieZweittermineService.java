...
public class CheckFreieZweittermineService {
	...
	public void analyseFreieZweittermine() {
		...
		LOG.debug(
			"VACME-ZWEITTERMINE: folgende OdI werden analysiert {}",
	...
	private void analyseFreieZweittermine(@NonNull OrtDerImpfung odi) {
		...
		if (firstFreierTermin == null) {
			LOG.debug(
				"VACME-ZWEITTERMINE: Es gibt keinen freien Ersttermin im OdI {} im Range von {} bis {}. Wir koennen "
					+ "abbrechen ",
				odi.getName(),
		...
		for (int i = 0; i < Constants.TOLERANZ_SPAN_DAYS; i++) {
			...
			if (!hasEnoughZweittermine(odi, analyziert)) {
				LOG.warn("VACME-ZWEITTERMINE: ODI {}: Für die freien Erstermine am Tag {} hat es keine passenden "
						+ "Zweittermine",
					odi.getName(),
					analyziert);
	...
	private boolean hasEnoughZweittermine(@NonNull OrtDerImpfung odi, @NonNull LocalDate datum) {
		...
		final long freiImpfung1 =
			impfterminRepo.getAnzahlFreieTermine(odi, Impffolge.ERSTE_IMPFUNG, datum, datum,
				KrankheitIdentifier.COVID);
		if (freiImpfung1 > 0) {
