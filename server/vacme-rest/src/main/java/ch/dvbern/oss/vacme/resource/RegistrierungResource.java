...
public class RegistrierungResource {
	...
	public RegistrierungsCodeJax registrieren(CreateRegistrierungJax createRegistrierungJax) {
		...
		// TODO Affenpocken: Aktuell aus Portal nur Covid-Dossier unterstuetzt
		final Impfdossier created = registrierungService.createImpfdossierCovid(fragebogen);
		return RegistrierungsCodeJax.from(created.getRegistrierung());
	}
	...
	public PersonalienJax getPersonalien(
		...
		// wir muessen hier aber auch damit umgehen koennen dass kein covid zertifikat existiert
		ImpfinformationDto infos = impfinformationenService.getImpfinformationen(
		...
		LocalDateTime timestampLetzterPostversand = zertifikatRunnerService.getTimestampOfLastPostversand(infos.getImpfdossier());
		final boolean abgeschlossenMitVollstaendigemImpfschutz = infos.getImpfdossier().abgeschlossenMitVollstaendigemImpfschutz();

		return new PersonalienJax(registrierung, hasCovidZertifikat, deservesZertifikat, timestampLetzterPostversand, abgeschlossenMitVollstaendigemImpfschutz);
	}
	...
	private Response updateSelfserviceEditDataBasic(@NonNull Registrierung registrierung, @NonNull SelfserviceEditJax personalienJax) {
		Fragebogen fragebogen =
			fragebogenService.findFragebogenByRegistrierungsnummer(registrierung.getRegistrierungsnummer());
		authorizer.checkUpdateAuthorization(registrierung);
