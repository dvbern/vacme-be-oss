...
public class MailService implements IDocumentQueueJobFinishedBenachrichtigungService {
	...
	private String appendStufeAndKantonToSubject(@NonNull String subject) {
		final String prefix =
			String.format("VACME-%s (%s): ", vacmeSettingsService.getMandant(), vacmeSettingsService.getStufe());
	...
	public void sendTextMail(
		...
		try {
			mailer.send(Mail.withText(to, appendStufeAndKantonToSubject(subject),
				appendInfoTestsystemToMessage(text)));
	...
	public void sendHtmlMail(
		...
		try {
			mailer.send(Mail.withHtml(to, appendStufeAndKantonToSubject(subject),
				appendInfoTestsystemToMessage(html)));
		} catch (final Exception e) {
			if (abortInCaseOfException) {
				throw new AppFailureException("Error while sending Mail to: '" + to + '\'', e);
			} else {
	...
	public void sendEinladungFachapplikation(@NonNull OdiUserJax userJax, @NonNull @NotNull OrtDerImpfungJax odi) {
		...
		String content = translate.translate(
			"mail_einladung_fachapplikation_content",
			username,
			vacmeSettingsService.getUrlFachapplikation(),
			linkBlog,
			kontaktFachlich,
			kontaktTechnisch);
	...
	private String getBlogLink(@NonNull OrtDerImpfungTyp typ) {
		String link = vacmeSettingsService.getUrlBlogFachapplikation();
		if (!link.endsWith("/")) {
			link = link + '/';
		}
		switch (typ) {
	...
	public void sendCheckFreieZweittermineMail(
		...
		String ccMailZuwenigeZweittermine = vacmeSettingsService.getCcMailZuwenigeZweittermine();
	...
	public void sendFinishedDocumentQueueSuccessJobMail(
		...
		String fullLink = getFullLink(vacmeSettingsService.getUrlFachapplikation(), resultId);
	...
	private String calculateExpiryTime(@NonNull DocumentQueue abrechnungDocQueue) {
		LocalDateTime expiryDateTime = abrechnungDocQueue.getResultTimestamp().plus(
			vacmeSettingsService.getMaxDocumentqueueResultAgeSeconds(),
			ChronoUnit.SECONDS);
	...
	public void sendFinishedDocumentQueueSuccessJobMail(
		...
		final String content = ServerMessageUtil.getMessage("mail_document_job_finished_content",
			...
			getFullLink(vacmeSettingsService.getUrlFachapplikation(), resultId),
			calculateExpiryTime(docQueue));
