...
public class RegistrierungService {
	...
	public Registrierung createRegistrierung(@NonNull Fragebogen fragebogen) {
		...
		try {
			...
			if (e.getCause() instanceof ConstraintViolationException) {
				LOG.warn(
					"VACME-WARNING: Es wurde versucht, eine Registrierung zweimal zu speichern von Benutzer: {}",
					registrierung.getBenutzerId());
	...
	public void updateSelfserviceData(
		@NonNull Fragebogen fragebogen,
		@NonNull Registrierung registrierung,
		@NonNull SelfserviceEditJax updateJax) {
	...
	public void deleteRegistrierung(@NonNull Registrierung registrierung, @NonNull Fragebogen fragebogen) {
		...
		if (impfinformationenService.hasAnyVacmeImpfungen(registrierung)) {
			throw AppValidationMessage.ILLEGAL_STATE.create("Registrierung "
				+ registrierung.getRegistrierungsnummer()
				+ " hat schon  VACME-Imfpungen");
		}
		final List<Zertifikat> allZertifikate =
			zertifikatService.getAllZertifikateRegardlessOfRevocation(registrierung);
		...
		LOG.info(
			"VACME-INFO: Fragebogen {} geloescht durch {}. RegistrierungsID {}",
			registrierung.getRegistrierungsnummer(),
	...
	private void validateOnlyOneEintragwithTermin(@NonNull Impfdossier impfdossier) {
		...
		if (eintraegeWithTermin > 1) {
			throw AppValidationMessage.ILLEGAL_STATE.create("Dossier " + impfdossier.getRegistrierung()
	...
	public void sendBenutzernameForRegistrierung(@NonNull String registrierungsnummer) {
		...
		final Optional<Registrierung> registrierungOptional =
			registrierungRepo.getByRegistrierungnummer(registrierungsnummer);
			...
			if (RegistrierungsEingang.ONLINE_REGISTRATION == registrierung.getRegistrierungsEingang()) {
				if (registrierung.getBenutzerId() != null) {
					final Optional<Benutzer> benutzerOptional =
						benutzerService.getById(Benutzer.toId(registrierung.getBenutzerId()));
					if (benutzerOptional.isPresent()) {
						...
						if (benutzer.getBenutzernameGesendetTimestamp() != null) {
							...
							long between = DateUtil.getMinutesBetween(
								benutzer.getBenutzernameGesendetTimestamp(),
								LocalDateTime.now());
	...
	private void runPriorityUpgradeToGroupForAge(int age, Prioritaet prioritaetFrom, Prioritaet prioritaetTo) {
		...
		for (String regnum : foundRegs) {
			Registrierung registrierung =
				registrierungRepo.getByRegistrierungnummer(regnum)
					.orElseThrow(() -> AppFailureException.entityNotFound(Registrierung.class, "regnum:" + regnum));
			registrierung.setPrioritaet(prioritaetTo);
		}
		LOG.info(
			"VACME-PRIORITY-UPDATE updated {} Regs that turned older than {} to Prio {} from {}",
			foundRegs.size(),
			age,
			prioritaetTo,
			prioritaetFrom);
