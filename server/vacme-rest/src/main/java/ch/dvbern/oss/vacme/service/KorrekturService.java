...
public class KorrekturService {
	...
	public void impfungKorrigieren(
		...
		final boolean needsNewZertifikat =
			ImpfungKorrekturJax.needsNewZertifikat(impfinformation.getImpfdossier(), korrekturJax.getImpffolge(),
				impfstoffFalschErfasst, impfstoffKorrigiert, impfinformation.getExternesZertifikat());
		final boolean needToRevoke = ImpfungKorrekturJax.needToRevoke(
			impfinformation.getImpfdossier(),
			korrekturJax.getImpffolge(),
			impfstoffFalschErfasst,
			impfstoffKorrigiert);
		...
		if (impfstoffKorrigiert.getAnzahlDosenBenoetigt() != impfstoffFalschErfasst.getAnzahlDosenBenoetigt()) {
			...
			LOG.warn(
				"VACME-KORREKTUR: Es wurde ein Impfstoff mit {} Dosen korrigiert auf einen Impfstoff mit {} Dosen. "
					+ "Registrierung {}",
			...
			if (korrekturJax.getImpffolge() == Impffolge.ERSTE_IMPFUNG) {
				...
				if (ImpfinformationenUtil.willBeGrundimmunisiertAfterErstimpfungImpfstoff(
					impfstoffKorrigiert,
					impfinformation.getExternesZertifikat())) {
					// Sicherstellen, dass keine zweite Impfung vorhanden ist
					if (impfinformation.getImpfung2() != null) {
						throw AppValidationMessage.IMPFUNG_ZUVIELE_DOSEN_KORREKTUR.create(
							impfstoffKorrigiert.getAnzahlDosenBenoetigt(),
							impfstoffFalschErfasst.getAnzahlDosenBenoetigt());
					}
				...
				if (ImpfinformationenUtil.willBeGrundimmunisiertAfterErstimpfungImpfstoff(
					impfstoffFalschErfasst,
					impfinformation.getExternesZertifikat())) {
					// Der alte Impfstoff hat nur eine Dosis benoetigt. Falls unterdessen bereits eine Booster-Impfung
					// besteht, waere diese
		...
		resendToSchnittstellenAndRegenerateArchive(impfung);
		...
		if (needToRevoke) {
			LOG.info(
				"VACME-ZERTIFIKAT-REVOCATION: Zertifikat der Impfung {} wird revoked fuer Registrierung {}, da der "
					+ "Impfstoff korrigiert wurde",
				impfung.getId(),
				registrierung.getRegistrierungsnummer());
		...
		LOG.info(
			"VACME-KORREKTUR: Fuer die Registrierung {} wurde eine Korrektur des Impfstoffs vorgenommen {} durch {}",
			registrierung.getRegistrierungsnummer(),
			korrekturJax,
		...
		boosterService.recalculateImpfschutzAndStatusmovesForSingleRegWithReload(
			registrierung,
			impfinformation.getKrankheitIdentifier());
	}
	...
	public void impfungOdiKorrigieren(
		...
		if (impftermin.getImpffolge() == Impffolge.ERSTE_IMPFUNG
			&& !ImpfdossierStatus.getMindestensGrundimmunisiertOrAbgeschlossen()
			...
			LOG.warn(
				"VACME-KORREKTUR: Fuer die Registrierung {} wurde der zweite Termin geloescht",
				registrierung.getRegistrierungsnummer());
		}
		...
		// Wir rufen hier die Methode direkt auf, um die Validierung zu umgehen! (im Repo wird validiert dass keine
		// Impfung am Termin haengt)
		...
		case BOOSTER_IMPFUNG:
			...
			Impfdossiereintrag eintrag =
				impfinformationDto.getImpfdossier().getEintragForImpffolgeNr(korrekturJax.getImpffolgeNr());
		...
		impfungRepo.update(impfung);
		LOG.info(
			"VACME-KORREKTUR: Fuer die Registrierung {} wurde eine Korrektur des ODIs vorgenommen {} durch {}",
			registrierung.getRegistrierungsnummer(),
			korrekturJax,
			this.userPrincipal.getBenutzerOrThrowException().getBenutzername());
	}
	...
	public void impfungSelbstzahlendeKorrigieren(
		...
		LOG.info(
			"VACME-KORREKTUR: Fuer die Registrierung {} wurde eine Korrektur des Zahlungsmodus vorgenommen {} durch "
				+ "{}",
			registrierung.getRegistrierungsnummer(),
			korrekturJax,
			this.userPrincipal.getBenutzerOrThrowException().getBenutzername());
	...
	public void impfungVerabreichungKorrigieren(
		...
		// Die korrigierten Daten werden weder zu VMDL noch zu MyCovidVac geschickt, es muss also nichts
		...
		LOG.info(
			"VACME-KORREKTUR: Fuer die Registrierung {} wurde eine Korrektur der Verabreichung vorgenommen {} durch "
				+ "{}",
			registrierung.getRegistrierungsnummer(),
			korrekturJax,
			this.userPrincipal.getBenutzerOrThrowException().getBenutzername());
	...
	public void impfungLoeschen(
		...
		if (impffolge == Impffolge.ERSTE_IMPFUNG) {
			...
			if (currentMaxImpfungNr != impffolgeNr) {
				throw AppValidationMessage.IMPFUNG_LOESCHEN_N.create(
					impffolgeNr,
					currentMaxImpfungNr,
					registrierung.getRegistrierungsnummer());
			}
	...
	private void impfung1Loeschen(@NonNull ImpfinformationDto infos) {
		if (infos.getImpfdossier().getBuchung().getImpftermin1() != null) {
			if (infos.getImpfung2() != null || CollectionUtils.isNotEmpty(infos.getBoosterImpfungen())) {
				throw AppValidationMessage.IMPFUNG_LOESCHEN_ERSTE.create(infos.getRegistrierung()
			...
			// Aber die Impfung 1 kann nur als letzte geloescht werden, d.h. wir haben nach dem Loeschen sicher kein
			// Zertifikt mehr
			// und koennen daher alles revozieren
			zertifikatService.queueForRevocation(infos.getImpfdossier());
	...
	private void impfung2Loeschen(@NonNull ImpfinformationDto infos) {
		if (infos.getImpfdossier().getBuchung().getImpftermin2() != null) {
			...
			if (CollectionUtils.isNotEmpty(infos.getBoosterImpfungen())) {
				throw AppValidationMessage.IMPFUNG_LOESCHEN_ZWEITE.create(infos.getRegistrierung()
					.getRegistrierungsnummer());
			}
			...
			unlinkZertifikatFromImpfungAndRevocateZertifikat(infos.getRegistrierung(), impfung);
			// Erst ab 14.0.0 koennen wir sicher sein, dass die Impfung verknuepft ist mit dem Zertifikat
			// Aber die Impfung 2 kann nur als letzte geloescht werden, d.h. wir haben nach dem Loeschen sicher kein
			// Zertifikt mehr
			// und koennen daher alles revozieren
			zertifikatService.queueForRevocation(infos.getImpfdossier());
	...
	private void impfungNLoeschen(
		...
		if (impftermin != null) {
			...
			// Eventuell bereits vorhandene Zertifikate muessen storniert werden
			// Bei Booster Impfungen koennen wir NICHT zwingend davon ausgehen, dass jetzt kein gueltiges Zertifikat
			// mehr bestehen darf.
			...
			ImpfdossierStatus impfdossierStatus =
				impfdossierService.ermittleLetztenDossierStatusVorKontrolleBooster(
					impfinformationDto.getImpfdossier(),
					impfdossiereintrag);
	...
	private void unlinkZertifikatFromImpfungAndRevocateZertifikat(
		...
		zertifikatList.forEach(zertifikat -> {
				LOG.info(
					"VACME-ZERTIFIKAT-REVOCATION: Zertifikat {} wird revoked fuer Registrierung {}, da eine "
				...
				// Dies, damit das Zertifikat nicht noch zum Download angeboten wird, bis die Revozierung ueber den
				// Batchjob durch ist
				// Gerade bei Postversand wird ja 3 Tage gewartet, damit das neue Zertifikat sicher schon da ist,
				// bevor das alte revoziert
				// wird, aber beim Loeschen der Impfung gibt es gar kein neues Zertifikat und somit kein Grund zu
				// warten.
	...
	private void impfungLoeschen(@NonNull ImpfinformationDto infos, @NonNull Impfung impfung) {
		...
		migrationRepo.getByImpfung(impfung).map(Migration::getId).map(Migration::toId).ifPresent(migrationRepo::delete);
		// delete the Impfung
		...
		if (vmdlService.wasSentToVMDL(impfung)) {
			// Send delete request to BAG VMDL if the Impfung was sent (Impfung.timestampVMDL != null) or was at any
			// moment in the past sent to VMDL (Impfung_AUD.timestampVMDL != null)
			// This guarantees that in case the Impfung was already sent once to VMDL and then corrected in VacMe
			// (which causes the timestampVMDL to become null again)
			// that this Impfung is deleted from VMDL even if the current timestampVMDL is null
			vmdlService.deleteImpfung(impfung);
		}

		// Impfung auch im Archivsystem als geloescht markieren
		d3ApiService.deleteInD3(impfung);

		LOG.info(
			"VACME-KORREKTUR: Fuer die Registrierung {} wurde die Impfung {} geloescht durch {}",
	...
	private VMDLServiceAbstract<? extends VMDLUploadBaseJax> getVmdlService(@NonNull Impfung impfung) {
		VMDLServiceAbstract<? extends VMDLUploadBaseJax> vmdlService =
			vmdlServiceFactory.createVMDLService(impfung.getTermin().getImpfslot().getKrankheitIdentifier());
		return vmdlService;
	}
	...
	private void impfungBoosterLoeschen(
		...
		if (impfinformationen.getKrankheitIdentifier().isSupportsVMDL()) {
			VMDLServiceAbstract<? extends VMDLUploadBaseJax> vmdlService = getVmdlService(impfung);
			if (vmdlService.wasSentToVMDL(impfung)) {
				// Send delete request to BAG VMDL if the Impfung was sent (Impfung.timestampVMDL != null) or was at
				// any moment in the past sent to VMDL (Impfung_AUD.timestampVMDL != null)
				// This guarantees that in case the Impfung was already sent once to VMDL and then corrected in VacMe
				// (which causes the timestampVMDL to become null again)
				// that this Impfung is deleted from VMDL even if the current timestampVMDL is null
				vmdlService.deleteImpfung(impfung);
		...
		LOG.info(
			"VACME-KORREKTUR: Fuer die Registrierung {} wurde die {} Impfung vom {} geloescht durch {}",
			registrierung.getRegistrierungsnummer(),
			impfinformationen.getImpfdossier().getKrankheitIdentifier() == KrankheitIdentifier.AFFENPOCKEN ?
				KrankheitIdentifier.AFFENPOCKEN.name() :
				KrankheitIdentifier.COVID.name() + " Booster-",
			impfung.getTimestampImpfung(),
			this.userPrincipal.getBenutzerOrThrowException().getBenutzername());
	}
	...
	public void impfungDatumKorrigieren(
		...
		Impftermin impftermin = impfung.getTermin();
		Registrierung registrierung = impfinformationDto.getRegistrierung();
		ImpfinformationenService.validateImpffolgeExists(
			korrekturJax.getImpffolge(),
		...
		final boolean needsNewZertifikat =
			korrekturJax.needsNewZertifikat(impfinformationDto, impfung.getTimestampImpfung());
		...
		// Wir rufen hier die Methode direkt auf, um die Validierung zu umgehen dass kein Termin freigegeben wird der
		// eine Impfung  hat!
		impftermin.setGebuchtFromImpfterminRepo(false);
		...
		if (needsNewZertifikat) {
			...
			if (needToRevoke) {
				LOG.info(
					"VACME-ZERTIFIKAT-REVOCATION: Zertifikat fuer Impfung {} wird revoked fuer Registrierung {}, "
		...
		impfungRepo.update(impfung);
		LOG.info(
			"VACME-KORREKTUR: Fuer die Registrierung {} wurde eine Korrektur des Datums vorgenommen {} durch {}",
			registrierung.getRegistrierungsnummer(),
			korrekturJax,
			this.userPrincipal.getBenutzerOrThrowException().getBenutzername());
	...
	public void personendatenKorrigieren(
		...
		// we need to check this before we change the values in the database so that we can check if the values have
		// changed
		...
		// Die KT_NACKDOKUMENTATION role kann handy und mail des Benutzerobjects anpassen deshalb ermoeglichen wir das
		// auch fuer die Registrierungsdaten
		if (registrierung.getRegistrierungsEingang() == RegistrierungsEingang.ONLINE_REGISTRATION
			...
			&& vacmeSettingsService.isEmailKorrekturEnabled()) {
		...
		for (ImpfinformationDto infos : infosPerImpfdossier) {
			...
			// Unter Umstaenden aendert sich mein vollstaendigerImpfschutzTyp aufgrund einer Anpassung des
			// immunsupprimiert-Flags:
			...
			if (infos.getKrankheitIdentifier().isSupportsZertifikat()) {
				...
				boolean needsNewZertifikat =
					korrekturJax.needsNewZertifikat(infos.getImpfdossier(), didInfoRelevantToZertifikatChange);
				...
				if (needsNewZertifikat) {
					...
					if (needToRevoke) {
						...
						LOG.info(
							"VACME-ZERTIFIKAT-REVOCATION: Alle Zertifikate werden revoked fuer Registrierung {}, da "
								+ "Personendaten veraendert wurden",
							registrierung.getRegistrierungsnummer());
		...
		LOG.info(
			"VACME-KORREKTUR: Fuer die Registrierung {} wurde eine Korrektur der Personendaten vorgenommen durch {}",
			registrierung.getRegistrierungsnummer(),
			this.userPrincipal.getBenutzerOrThrowException().getBenutzername());
	...
	public void updateEmailTelephone(@NonNull UUID benutzerId, @NonNull EmailTelephoneKorrekturJax korrekturJax) {
		...
		keyCloakService.updateUserLoginDataForRegistrierung(
			benutzer,
			korrekturJax.getMail(),
			korrekturJax.getTelefon());
	...
	private void resendImpfungenToVMDLAndRegenArchiveIfNecessary(@Nullable ImpfinformationDto infos) {
		...
		final List<Impfung> impfungen =
			ImpfinformationenUtil.getImpfungenOrderedByImpffolgeNr(infos);
		impfungen.forEach(this::regenerateArchiveIfNecessary);
	}

	private void resendToSchnittstellenAndRegenerateArchive(@NonNull Impfung impfung) {
		resendImpfungVMDL(impfung);
		regenerateArchiveIfNecessary(impfung);
	}

	private void regenerateArchiveIfNecessary(@NonNull Impfung impfung) {
