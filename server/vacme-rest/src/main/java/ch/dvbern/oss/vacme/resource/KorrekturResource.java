...
public class KorrekturResource {
	...
	private static void assertThatAtLeastOneImpfungIsEditable(
		...
		if (korrekturDashboardJax.getImpfung1IfEditableForRole() == null
			...
			LOG.warn("Kantonsuser tried to edit  Registrierung {}, which has no Impfungen that he can modify", korrekturDashboardJax.getRegistrierungsnummer());
	...
	public Response impfungKorrigieren(
		...
		return basicImpfungKorrigieren(regNummer,
	...
	public Response impfungLoeschen(
		...
		return basicImpfungKorrigieren(regNummer,
			...
			korrekturJax.getImpffolgeNr(),
			korrekturJax.getKrankheitIdentifier(),
			(impfung, impfinformationDto) -> this.korrekturService.impfungLoeschen(impfinformationDto,
				korrekturJax.getImpffolge(),
				korrekturJax.getImpffolgeNr()));
	}
	...
	public Response impfungVerabreichungKorrigieren(
		...
		return basicImpfungKorrigieren(regNummer,
			...
			korrekturJax.getImpffolgeNr(),
			korrekturJax.getKrankheitIdentifier(),
			(impfung, impfinformation) -> this.korrekturService.impfungVerabreichungKorrigieren(korrekturJax,
				impfung,
				impfinformation));
	}
	...
	private void checkHasNoNonRevokedZertifikat(@NonNull Registrierung registrierung) {
		...
		if (hasAnyZertifikat) {
			throw AppValidationMessage.REGISTRIERUNG_ZERTIFIKAT_ALREADY_GENERATED.create(registrierung.getRegistrierungsnummer(),
	...
	public Response regenerateZertifikate(
		...
		ZertifikatResultDto result =
			zertifikatRunnerService.createZertifikatForRegistrierung(registrierung.getRegistrierungsnummer(),
				idOfNewestImpfung,
				type);
		if (!result.success) {
