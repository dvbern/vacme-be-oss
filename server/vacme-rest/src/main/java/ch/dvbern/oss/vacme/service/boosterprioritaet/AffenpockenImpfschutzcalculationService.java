...
public class AffenpockenImpfschutzcalculationService implements ImpfschutzCalculationService {
	...
	private void addCommonRules() {
		final AffenpockenImpfschutzcalculationConfigDTO dto =
			vacmeSettingsService.getAffenpockenImpfschutzcalculationConfigDTO();
		// rule that handels all cases,
		rules.add(AffenpockenImpfschutzRule.createMinAgeRule(
			specifiedImpfstoffe,
			dto.getMinAge(),
			dto.getFreigabeOffsetImpfungWithoutImpfschutz(), dto.getFreigabeoffsetLetzteImpfungWithoutImpfschutzUnit(),
			dto.getFreigabeOffsetImpfungWithImpfschutz(), dto.getFreigabeoffsetLetzteImpfungWithImpfschutzUnit(),
			0, null
		));
