...
public class WellApiService {
	...
	// todo VACME-2659: MIK-API call when unlinking
	@CheckIfWellDisabledInterceptor
	...
	void deleteAccountLinkAsync(@NonNull UUID wellId) {
		Uni.createFrom().item(wellId)
			...
			.subscribe().with(
				item -> {
					LOG.debug(String.format("WELL-API: Unlinked wellId '%s'", wellId));
				}, throwable -> {
	...
	private static boolean terminIsInFuture(@NonNull Impftermin impftermin) {
		...
		boolean isInFuture = terminStart.isAfter(LocalDateTime.now());
		return isInFuture;
	}
