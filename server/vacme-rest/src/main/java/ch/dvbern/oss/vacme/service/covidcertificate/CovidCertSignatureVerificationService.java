...
public class CovidCertSignatureVerificationService {
	...
	private void initCert() {
		if (certificate == null) {
			try {
				certificate = CovidCertKeyUtil.loadCert(vacmeSettingsService.getCovidCertApiPrivateKeyPath(), vacmeSettingsService.getCovidCertApiPassword(), vacmeSettingsService.getCovidCertApiKeyAlias());
