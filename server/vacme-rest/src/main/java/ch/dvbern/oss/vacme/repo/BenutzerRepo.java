...
public class BenutzerRepo {
	...
	public Optional<Benutzer> getByBenutzernameFromRegapp(@NonNull String benutzername) {
		return this.getByBenutzername(benutzername, this.vacmeSettingsService.getKeycloakRegIssuerUrl());
	}
	...
	public Optional<Benutzer> getByBenutzername(@NonNull String benutzername, @NonNull String keycloakIssuer) {
		...
		List<Benutzer> collect =
			fetchedUsers.stream()
				.filter(benutzer1 -> benutzer1.getIssuer().endsWith(keycloakIssuer))
				.collect(Collectors.toList());

		if (collect.size() > 1) {
			throw new NonUniqueResultException("Found more than one Benutzer issued from "
				+ keycloakIssuer
				+ " for username "
				+ benutzername);
		}
	...
	public List<ID<Benutzer>> getAllActiveSubordinateBenutzerWhereLastSessionAndLastUnlockedIsOlderThanMinutesOrNull(
		...
		SmartJPAQuery<UUID> query =
			db.select(aliasBenutzer.id)
				...
				.distinct()
				.where(
					(aliasBenutzer.timestampLastSessionId.lt(cutoffDateTimeForOldSessions)
						.or(aliasBenutzer.timestampLastSessionId.isNull()))
						...
						.and(aliasBenutzer.id.notIn(db.select(otherBenutzer.id)
							...
							.where(QBenutzerBerechtigung.benutzerBerechtigung.rolle.in(
								BenutzerRolle.OI_IMPFVERANTWORTUNG,
								BenutzerRolle.OI_ORT_VERWALTER))
							.asSubQuery()))
	...
	private Predicate getConditionLastUnlocked(LocalDateTime cutoffDateTimeForOldSessions, QBenutzer aliasBenutzer) {
		if (MandantUtil.getMandant().equals(Mandant.BE)) {
			...
			return aliasBenutzer.timestampLastUnlocked.lt(cutoffDateTimeForOldSessions)
				.or(aliasBenutzer.timestampLastUnlocked.isNull());
		}
	}
}
