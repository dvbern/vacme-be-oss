...
public abstract class VMDLRequestAuthTokenFactory implements ClientHeadersFactory {
	...
	public MultivaluedMap<String, String> update(MultivaluedMap<String, String> incomingHeaders, MultivaluedMap<String, String> clientOutgoingHeaders) {
		...
		if (expiration == null || !expiration.isAfter(LocalDateTime.now())) {
			VMDLAuthResponseJax response = vmdlAuthRestClientService.aquireToken(
				vacmeSettingsService.getVmdlTenantID(),
				grantType,
				vacmeSettingsService.getVmdlUsername(),
				getClientID(),
				vacmeSettingsService.getVmdlPassword(),
				scope);
