...
public class StatsRepo {
	...
	public List<StatsTerminAndImpfungJax> getImpfTermineAndImpfungenForOdi(
		...
		return db.selectFrom(QImpftermin.impftermin)
			...
			.where(QImpftermin.impftermin.impfslot.ortDerImpfung.eq(ortDerImpfung)
				.and(QImpftermin.impftermin.impfslot.zeitfenster.bis.between(
					dateVon.atTime(LocalTime.MIN),
					dateBis.atTime(LocalTime.MAX)))
			)
			.fetch();
	}
	...
	public long getAnzahlCovidImpfungNDurchgefuehrt() {
		return db.select(QImpfung.impfung)
			...
			.on(QImpftermin.impftermin.eq(QImpfdossiereintrag.impfdossiereintrag.impftermin))
	...
	public long getAnzahlRegistrierungenCallcenterWithTermin() {
		SmartJPAQuery<Impfdossier> query = db
			...
			.where(registrierung.registrierungsEingang.eq(RegistrierungsEingang.CALLCENTER_REGISTRATION)
				.and(QImpfdossier.impfdossier.buchung.impftermin1.isNotNull()
					.or(QImpfdossier.impfdossier.buchung.impftermin2.isNotNull())));
		return query.fetchCount();
	}
	...
	public long getPendentImpfung1(
		...
		SmartJPAQuery<Impfdossier> query = db.selectFrom(QImpfdossier.impfdossier)
			...
			.on(QImpfslot.impfslot.id.eq(termin.impfslot.id))
			.innerJoin(QOrtDerImpfung.ortDerImpfung)
			.on(QOrtDerImpfung.ortDerImpfung.id.eq(termin.impfslot.ortDerImpfung.id))
			.where(QImpfdossier.impfdossier.krankheitIdentifier.eq(krankheitIdentifier)
				.and(termin.impfslot.ortDerImpfung.id.eq(ortDerImpfung.getId())
					.and(termin.impfslot.zeitfenster.bis.between(
						datum.atTime(LocalTime.MIN),
						datum.atTime(LocalTime.MAX)))
	...
	public long getPendentImpfung2ImpfstoffEmpfohlen(
		...
		SmartJPAQuery<Impfdossier> query = db.selectFrom(QImpfdossier.impfdossier)
			...
			.innerJoin(QImpfslot.impfslot)
			.on(QImpfslot.impfslot.id.eq(termin2.impfslot.id))
			.innerJoin(QOrtDerImpfung.ortDerImpfung)
			.on(QOrtDerImpfung.ortDerImpfung.id.eq(termin2.impfslot.ortDerImpfung.id))
			.innerJoin(QImpfung.impfung)
			.on(QImpfung.impfung.termin.id.eq(termin1.id))
			.innerJoin(QImpfstoff.impfstoff)
			.on(QImpfstoff.impfstoff.id.eq(QImpfung.impfung.impfstoff.id))
			.where(QImpfdossier.impfdossier.krankheitIdentifier.eq(krankheitIdentifier)
				.and(termin2.impfslot.ortDerImpfung.id.eq(ortDerImpfung.getId())
					.and(termin2.impfslot.zeitfenster.bis.between(
						datum.atTime(LocalTime.MIN),
						datum.atTime(LocalTime.MAX)))
	...
	public long getPendentImpfungN(
		...
		SmartJPAQuery<Registrierung> query = db
			...
			.on(QImpfung.impfung.termin.id.eq(termin.id))
			.leftJoin(QImpfschutz.impfschutz)
			.on(QImpfschutz.impfschutz.eq(QImpfdossier.impfdossier.impfschutz))
			.where(termin.impfslot.ortDerImpfung.id.eq(ortDerImpfung.getId())
				...
				.and(QImpfslot.impfslot.zeitfenster.bis.between(
					datum.atTime(LocalTime.MIN),
					datum.atTime(LocalTime.MAX)))
	...
	public long getDurchgefuerteImpfung1(Impfstoff impfstoff, OrtDerImpfung ortDerImpfung, LocalDate datum) {
		...
		SmartJPAQuery<Impfdossier> query = db
			...
			.where(termin.impfslot.ortDerImpfung.id.eq(ortDerImpfung.getId())
				.and(QImpfstoff.impfstoff.id.eq(impfstoff.getId()))
				.and(QImpfung.impfung.timestampImpfung.between(
					datum.atTime(LocalTime.MIN),
					datum.atTime(LocalTime.MAX))));
		return query.fetchCount();
	}
	 ...
	 */
	public List<ImpfzentrumDayStatTermin1DataRow> getTagesstatistikDatenTermin1(
	 ...
	 */
	public List<ImpfzentrumDayStatTermin2DataRow> getTagesstatistikDatenTermin2(
		OrtDerImpfung ortDerImpfung,
		LocalDate datum) {
		...
		List<ImpfzentrumDayStatTermin2DataRow> termine2AtOdI = db
			...
			.where(aliasTermin2.impfslot.zeitfenster.von.between(
					datum.atTime(LocalTime.MIN),
					datum.atTime(LocalTime.MAX))
	 ...
	 * @return transformiertes
	 */
	public List<ImpfzentrumDayStatTerminNDataRow> getTagesstatistikDatenTerminN(
		OrtDerImpfung ortDerImpfung,
		LocalDate datum) {
