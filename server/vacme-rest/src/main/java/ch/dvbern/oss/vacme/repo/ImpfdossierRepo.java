...
public class ImpfdossierRepo {
	...
	public List<String> findRegsWithoutImpfdossierCovid() {
		return db.select(QRegistrierung.registrierung.registrierungsnummer)
			...
			.leftJoin(QImpfdossier.impfdossier)
			.on(QImpfdossier.impfdossier.registrierung.eq(QRegistrierung.registrierung))
			.where(QImpfdossier.impfdossier.id.isNull()
				.or(QImpfdossier.impfdossier.id.isNotNull()
					.and(QImpfdossier.impfdossier.krankheitIdentifier.ne(KrankheitIdentifier.COVID))))
			.fetch();
	}
