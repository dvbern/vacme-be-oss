...
public class ImpfslotService {
	...
	public void createEmptyImpfslots(@NonNull OrtDerImpfung ortDerImpfung, @NonNull LocalDateTime startDay, int monthsAmount) {
		final ImpfslotCreationSettingsDTO settingsDTO =
			vacmeSettingsService.getImpfslotCreationSettingsDTO();
		Duration slotDuration = Duration.ofMinutes(settingsDTO.getDuration());
		Predicate<LocalDateTime> filter = getDailyFilter(settingsDTO);
	...
	protected Predicate<LocalDateTime> getDailyFilter(@NonNull ImpfslotCreationSettingsDTO settingsDTO) {
		ValueRange zeitraum = ValueRange.of(
			LocalDate.now().atTime(settingsDTO.getStart(), 0).get(ChronoField.MINUTE_OF_DAY),
			LocalDate.now().atTime(settingsDTO.getEnd(), 0).minusMinutes(1).get(ChronoField.MINUTE_OF_DAY));
