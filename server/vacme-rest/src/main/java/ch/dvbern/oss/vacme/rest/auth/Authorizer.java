...
public class Authorizer {
	...
	public void checkUpdateAuthorization(@Nullable Impfdossier impfdossier) {
		...
		if (impfdossier != null) {
			if (impfdossier.getRegistrierung().getTimestampArchiviert() != null) {
				LOG.error("Registrierung {} ist bereits archiviert. Es koennen keine Updates mehr gemacht werden",
					impfdossier.getRegistrierung().getRegistrierungsnummer());
				throw AppValidationMessage.REGISTRIERUNG_ALREADY_ARCHIVED.create(impfdossier.getRegistrierung().getRegistrierungsnummer());
			}
			// Ansonsten gleich wie read
