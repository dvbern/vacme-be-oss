...
public class ApplicationHealthRepo {
	...
	public List<RegistrierungTermineImpfungJax> getFreieTermineMitGebuchtFlag() {
		...
		final List<RegistrierungTermineImpfungJax> result = db
			...
			.where(QImpftermin.impftermin.gebucht.isTrue()
				.and(
					JPAExpressions.selectOne().from(QImpfdossier.impfdossier)
						.where(QImpfdossier.impfdossier.buchung.impftermin1.id.eq(QImpftermin.impftermin.id)).notExists()
				)
				.and(
					JPAExpressions.selectOne().from(QImpfdossier.impfdossier)
						.where(QImpfdossier.impfdossier.buchung.impftermin2.id.eq(QImpftermin.impftermin.id)).notExists()
				)
				.and(
		...
		try {
			...
			if (result.isEmpty()) {
				...
				for (Impfdossier impfdossier : result) {
					resultDTO.addInfo(MessageFormat.format(
						"Covid Dossier fuer Registrierung {0}, im Status {1}  ohne Impfungen und ohne ExternesZertifikat hat trotzdem das Flag vollstaendigGeimpft gesetzt",
	...
	public ResultDTO getHealthCheckDoppeltGeimpftOhneVollsaendigerImpfschutz() {
		...
		try {
			...
			if (result.isEmpty()) {
				...
				for (Impfdossier impfdossier : result) {
					resultDTO.addInfo(MessageFormat.format(
						"Covid Dossier fuer Reg {0}, im Status {1}  mit zwei Impfungen hat trotzdem das Flag vollstaendigGeimpft NICHT gesetzt",
						impfdossier.getRegistrierung().getRegistrierungsnummer(),
						impfdossier.getDossierStatus()));
				}
				resultDTO.finish(false);
	...
	public ResultDTO getHealthCheckAbgeschlossenOhneVollsaendigerImpfschutz() {
		...
		try {
			...
			if (result.isEmpty()) {
				...
				for (Impfdossier impfdossier : result) {
					resultDTO.addInfo(MessageFormat.format(
						"Covid Impfdossier fuer Reg {0}, im Status Abgeschlossen hat trotzdem das Flag vollstaendigGeimpft NICHT gesetzt",
						impfdossier.getRegistrierung().getRegistrierungsnummer()));
				}
				resultDTO.finish(false);
			}
	...
	public ResultDTO getHealthCheckNichtAbgeschlossenAberVollstaendigerImpfschutz() {
		...
		String query = "SELECT I.* FROM Registrierung R inner join Impfdossier I On I.registrierung_id = R.id "
			+ "WHERE I.dossierStatus not in ('ABGESCHLOSSEN', 'ABGESCHLOSSEN_OHNE_ZWEITE_IMPFUNG', 'IMMUNISIERT', 'FREIGEGEBEN_BOOSTER', 'ODI_GEWAEHLT_BOOSTER', 'GEBUCHT_BOOSTER', 'KONTROLLIERT_BOOSTER') "
	...
	public ResultDTO getHealthCheckAbgeschlossenOhneCoronaAberVollstaendigerImpfschutz() {
		...
		try {
			...
			if (result.isEmpty()) {
				...
				for (Impfdossier impfdossier : result) {
					resultDTO.addInfo(MessageFormat.format(
						"Covid-Dossier fuer Registrierung {0}, im Status {1} ist abgeschlossen ohne Corona, hat aber trotzdem das Flag vollstaendigGeimpft gesetzt",
						impfdossier.getRegistrierung().getRegistrierungsnummer(),
						impfdossier.getDossierStatus()));
				}
				resultDTO.finish(false);
	...
	public ResultDTO getRegistrierungenOhneCovidDossier() {
		...
		String query = "select * from Registrierung where id not in (select registrierung_id from Impfdossier where krankheitIdentifier = 'COVID');";
		try {
			final Query nativeQuery = db.getEntityManager().createNativeQuery(query, Registrierung.class);
			nativeQuery.setHint(DB_QUERY_TIMEOUT_HINT, dbQueryTimeoutInSeconds);
