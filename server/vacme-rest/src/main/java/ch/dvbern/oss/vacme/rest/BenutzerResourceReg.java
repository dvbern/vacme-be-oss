...
public class BenutzerResourceReg {
	...
	private boolean isCookieSecureQ() {
		final boolean forceCookieSecureFlag = vacmeSettingsService.isCookieSecure();
	...
	public List<String> readFederationLinks(
	) {
		return this.benutzerService.readFederationLinksFromCurrentUser();
	}

	@DELETE
	@NonNull
	@NotNull
	@Path("/federation/{idpalias}")
	@RolesAllowed(IMPFWILLIGER)
	public List<String> removeFederationLink(
		@PathParam("idpalias") @NonNull String idpalias
	) {
		this.benutzerService.removeFederationLinkFromCurrentUser(idpalias);
		return this.benutzerService.readFederationLinksFromCurrentUser();
	}
}
