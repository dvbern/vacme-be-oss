...
public class FTPClientService {
	...
	private void __open() throws IOException {
		final FTPClientConfigDTO ftpClientConfigDTO = vacmeSettingsService.getFtpClientConfigDTO();
		...
		ftps.connect(ftpClientConfigDTO.getFtpServer(), ftpClientConfigDTO.getFtpPort());
		...
		reply = ftps.sendCommand("HOST", ftpClientConfigDTO.getFtpServer()); //wegen virtueller Host
		...
		boolean loggedin = ftps.login(ftpClientConfigDTO.getFtpUsername(), ftpClientConfigDTO.getFtpPassword());
		if (!loggedin) {
			ftps.disconnect();
	...
	public Collection<String> listFiles(@NonNull String path) {
		if (vacmeSettingsService.getFtpClientConfigDTO().isFtpDisabled()) {
