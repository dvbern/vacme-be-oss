...
public class ImpfdossierFileRepo {
	...
	@Nullable
	public ImpfdossierFile getImpfdossierFile(
	...
	public long getAnzahlCallcenterDokumente(
		@NonNull ImpfdossierFileTyp typ,
		@NonNull LocalDate erstelltAm
	) {
		return getAnzahlCallcenterDokumenteVonBis(
			typ,
			erstelltAm.atStartOfDay(),
			erstelltAm.plusDays(1).atStartOfDay());
	}

	public long getAnzahlCallcenterDokumenteVonBis(
		@NotNull ImpfdossierFileTyp typ,
		@NotNull LocalDateTime von,
		@NotNull LocalDateTime bis
	) {
		// Nur diejenigen ImpfdossierFiles, welche eine Krankheit betreffen die vom Callcenter behandelt wird
		final List<KrankheitIdentifier> krankheitenThatSupportCallcenter = Arrays.stream(KrankheitIdentifier.values())
			.filter(KrankheitIdentifier::isSupportsCallcenter)
			.collect(
				Collectors.toList());
		QRegistrierung aliasRegistrierung = new QRegistrierung("registrierung");
		return db.select(QImpfdossierFile.impfdossierFile)
			.from(QImpfdossierFile.impfdossierFile)
			.innerJoin(QImpfdossierFile.impfdossierFile.impfdossier.registrierung, aliasRegistrierung)
			.where(QImpfdossierFile.impfdossierFile.fileTyp.eq(typ)
				.and(aliasRegistrierung.registrierungsEingang.eq(RegistrierungsEingang.CALLCENTER_REGISTRATION))
				.and(QImpfdossierFile.impfdossierFile.timestampErstellt.between(von, bis))
				.and(QImpfdossierFile.impfdossierFile.impfdossier.krankheitIdentifier.in(krankheitenThatSupportCallcenter))
			)
			.fetchCount();
	}
