...
public class CovidCertApiService {
	...
	public Zertifikat createVaccinationCert(
		...
		Zertifikat zertifikat = new Zertifikat(infos.getImpfdossier());
		...
		try {
			CovidCertificateCreateResponseDto vaccinationCert =
				this.covidCertRestApiClientService.createVaccinationCert(vaccCertCreateDto);
			...
			if (e.getErrorFromApi() != null) {
				LOG.error(
					"VACME-ZERTIFIKAT: Covic-Cert Error Status: '{}', message: '{}'",
					e.getErrorFromApi().getErrorCode(),
					e.getErrorFromApi().getErrorMessage());
			} else {
				LOG.error("VACME-ZERTIFIKAT: Unknown Error from CovidCert Api ", e);
			}
			LOG.error(
				"VACME-ZERTIFIKAT: there was an error when trying to create a certificate. Detailinformation: \n'{}'",
	...
	public void revokeVaccinationCert(
		...
		try {
			...
			LOG.info(
				"VACME-ZERTIFIKAT: Certificate with uvci {} for Registration {} has been revoked",
			...
			if (e.getErrorFromApi() != null) {
				...
				LOG.error(
					"VACME-ZERTIFIKAT: Covic-Cert Revocation Error Status: '{}', message: '{}'",
					errorCode,
					e.getErrorFromApi().getErrorMessage());
				if (CovidCertErrorcode.CC_ERROR_DUPLICATE_UVCI.getErrorCode() == errorCode) {
					// Zertifikat scheint bereits revoziert zu sein. Wir setzen es auf revoziert (bzw. werfen keine
					// Exception)
					LOG.info(
						"VACME-ZERTIFIKAT: Received CC_ERROR_DUPLICATE_UVCI for Revocation uvci {} and Registration {}"
							+ ". Setting to revoked.",
						revocationDto.getUvci(),
						zertifikat.getRegistrierung().getRegistrierungsnummer());
					return;
			...
			LOG.error(
				"VACME-ZERTIFIKAT: there was an error when trying to revoke a certificate. Detailinformation: \n'{}'",
	...
	VaccinationCertificateCreateDto mapRegForCovidCertDto(
		...
		Objects.requireNonNull(
			impfung,
			"Eine VacMe-Impfung muss existieren " + registrierung.getRegistrierungsnummer());
		...
		if (batchType == CovidCertBatchType.POST) {
			try {
				...
				LOG.info(
					"VACME-ZERTIFIKAT: Could not map '{}' as a valid siwss-plz for registrierung {}. Letter will not "
						+ "be sent",
	...
	private CovidCertificatePersonNameDto mapName(@NotNull Registrierung reg) {
		...
		if (reg.getVorname().length() > Constants.MAX_NAME_LENGTH_COVIDCERT) {
			LOG.warn(
				"VACME-ZERTIFIKAT-KUERZUNG Vorname musste abgekuerzt werden fuer {}",
		...
		if (reg.getName().length() > Constants.MAX_NAME_LENGTH_COVIDCERT) {
			LOG.warn(
				"VACME-ZERTIFIKAT-KUERZUNG Familienname musste abgekuerzt werden fuer {}",
				reg.getRegistrierungsnummer());
	...
	private VaccinationCertificateDataDto mapImpfungForCovidCertDTO(
		...
		if (StringUtils.isBlank(covidCertProdCode) || covidCertProdCode.equals("xxx")) {
			throw new AppFailureException("Covidcert Mapping for Impfstoff '"
				+ impfung.getImpfstoff()
				+ "' not yet implemented!");
		}
