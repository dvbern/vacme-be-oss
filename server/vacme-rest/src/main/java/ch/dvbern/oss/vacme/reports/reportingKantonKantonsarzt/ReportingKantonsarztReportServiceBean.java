...
public class ReportingKantonsarztReportServiceBean extends AbstractReportServiceBean {
	...
	@Inject
	public ReportingKantonsarztReportServiceBean(@NonNull Db db) {
		this.db = db;
	}
	...
	private String loadStatisticsQuery() {
		...
		InputStream inputStream = ReportingKantonsarztReportServiceBean.class.getResourceAsStream(statistikQueryFilename);
	...
	public StreamingOutput generateStatisticsExport() {
		return output -> {
			...
			try (
				...
				CSVPrinter printer = CSVFormat.DEFAULT.withHeader(StatistikReportingKantonsarztDTO.CSV_HEADER).print(writer);
			) {
	...
	private Stream<StatistikReportingKantonsarztDTO> runStatisticsQueryAsStream() {
		Query nativeQuery = this.db.getEntityManager().createNativeQuery(loadStatisticsQuery(), Constants.REPORTING_KANTONSARZT_DTO_MAPPING);
