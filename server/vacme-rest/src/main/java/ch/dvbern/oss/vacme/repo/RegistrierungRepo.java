...
public class RegistrierungRepo {
	...
	public long getAnzahlBoosterOrGrundimunisierungGT3Covid(@NonNull OrtDerImpfungTyp typ, @NonNull LocalDate von, @NonNull LocalDate bis) {
		var anzahl = db
			.selectFrom(impfung)
	...
	public long getAnzahlCovidBoosterOhneErstimpfungOderBoosterImKalenderjahr(
		...
		return db
			...
			.where(ortDerImpfung.typ.eq(typ)
				...
				.and(impfdossier.krankheitIdentifier.eq(KrankheitIdentifier.COVID))
				.and(impfdossier.notIn(getImpfdossierMitMehrerenBooster(firstDayOfYear, bis, KrankheitIdentifier.COVID).asSubQuery())))
			.fetchCount();
	}
