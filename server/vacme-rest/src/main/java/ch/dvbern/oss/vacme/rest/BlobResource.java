...
public class BlobResource {
	...
	public List<FileInfoJax> upload(
		...
		try {
			...
			impfdossierFiles.stream()
				...
				.findFirst()
				.ifPresent(file -> {
					throw AppValidationMessage.UPLOAD_INVALID_FILE_TYPE.create(file.getFileBlob()
						.getMimeType()
						.getMimeType());
				});
	...
	private boolean checkFiletypeAllowed(ImpfdossierFile fileInfo) {
		MimeType contentType = fileInfo.getFileBlob().getMimeType();
		return this.readMimeTypeWhitelist().contains(contentType.getMimeType());
	}

	private Collection<String> readMimeTypeWhitelist() {
		return vacmeSettingsService.getWhitelistedMimeTypesForUpload();
	}
