...
public class ImpfdossierService {
	...
	protected int minAlterCovidImpfung;
	...
	@NonNull
	public List<String> findRegsWithoutImpfdossierCovid() {
		return impfdossierRepo.findRegsWithoutImpfdossierCovid();
	}
	...
	public void handleGueltigkeitKontrolleAbgelaufen(
		...
		case IMPFUNG_1_KONTROLLIERT:
			if (isKontrolleAbgelaufen(impfdossier.getImpfungkontrolleTermin1())) {
				...
				LOG.info("VACME-KONTROLLE-ABGELAUFEN: Kontrolle 1 abgelaufen. Setze Status zurueck auf {}, "
								+ "Registrierung {}",
					lastStatus,
		...
		case KONTROLLIERT_BOOSTER:
			...
			if (isKontrolleAbgelaufen(eintrag.getImpfungkontrolleTermin())) {
				...
				impfdossier.setDossierStatus(lastStatus);
				LOG.info("VACME-KONTROLLE-ABGELAUFEN: Kontrolle N abgelaufen. Setze Status zurueck auf {}, "
								+ "Registrierung {}",
					lastStatus,
					registrierung.getRegistrierungsnummer());
			}
	...
	private void moveFromFreigegebenToNeuIfMinAgeNotReached(@NonNull @NotNull Impfdossier impfdossier, int alter) {
		...
		if (ImpfdossierStatus.FREIGEGEBEN == impfdossier.getDossierStatus() && alter < minAlterCovidImpfung) {
	...
	public void handleResetSelbstzahlerFlagIfRegulaerFreigegeben(@NonNull Impfdossier impfdossier) {
		if (impfdossier.getBuchung().isSelbstzahler()) {
			if (impfdossier.getImpfschutz() != null
				&& impfdossier.getImpfschutz().getFreigegebenNaechsteImpfungAb() != null
				&& impfdossier.getImpfschutz().getFreigegebenNaechsteImpfungAb().isBefore(LocalDateTime.now())
			) {
				impfdossier.getBuchung().setSelbstzahler(false);
			}
		}
	}
}
