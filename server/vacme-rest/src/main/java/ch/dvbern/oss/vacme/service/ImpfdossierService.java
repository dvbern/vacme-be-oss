...
public class ImpfdossierService {
	...
	private boolean isKontrolleAbgelaufen(
		...
		if (impfungkontrolleTermin != null) {
			...
			final boolean kontrolleAbgelaufen = kontrolleSince > vacmeSettingsService.getKontrolleGueltigkeitHours() * 60;
	...
	private void moveFromFreigegebenToNeuIfMinAgeNotReached(@NonNull @NotNull Impfdossier impfdossier, int alter) {
		...
		if (ImpfdossierStatus.FREIGEGEBEN == impfdossier.getDossierStatus() && alter < vacmeSettingsService.getMinAlterCovidImpfung()) {
