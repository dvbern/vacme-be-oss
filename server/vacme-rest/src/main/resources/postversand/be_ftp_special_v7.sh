#!/bin/bash
#
# v3: 2021-07-02 - muje: Added Zertifikatsstorno Export
...
#DOKUMENTABSAGE=`mysql -Ns -D ${DATABASE} -e "SELECT registrierungsnummerFROM RegistrierungINNER JOIN Impfdossier I on Registrierung.id = I.registrierung_id INNER JOIN ImpfdossierFile F on I.id = F.impfdossier_id WHERE I.krankheitIdentifier = 'COVID' AND F.fileTyp LIKE 'TERMIN_ABSAGE' AND abgeholt = FALSE AND F.timestampErstellt < '${DATE}';"`
...
# We're currently not doing Dokumenteabsagen for Berne
