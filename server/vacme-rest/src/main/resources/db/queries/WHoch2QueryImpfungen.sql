(SELECT 'Registrierungs_ID', 'Ort_der_Impfung_ID', 'Ort_der_Impfung_Name', 'Ort_der_Impfung_GLN', 'Ort_der_Impfung_Typ',
		...
		'Impffolgenummer', 'Alter_bei_Termin')
UNION
(SELECT registrierung.id AS Registrierungs_ID, odi.identifier AS Ort_der_Impfung_ID, odi.name AS Ort_der_Impfung_Name,
		odi.glnNummer AS Ort_der_Impfung_GLN, odi.typ AS Ort_der_Impfung_Typ,
		DATE_FORMAT(slot.von, '%d.%m.%Y %T') AS Termin_Impfung,
		DATE_FORMAT(impfung.timestampImpfung, '%d.%m.%Y %T') AS Impfung_am, impfstoff.wHoch2Code AS Impfstoff_Name,
		...
		1 AS Impffolgenummer, TIMESTAMPDIFF(YEAR, registrierung.geburtsdatum, slot.von) AS Alter_bei_Termin
 ...
 WHERE impfung.termin_id IS NULL OR impfstoff.wHoch2Code IS NOT NULL)
UNION
(SELECT registrierung.id AS Registrierungs_ID, odi.identifier AS Ort_der_Impfung_ID, odi.name AS Ort_der_Impfung_Name,
		odi.glnNummer AS Ort_der_Impfung_GLN, odi.typ AS Ort_der_Impfung_Typ,
		DATE_FORMAT(slot.von, '%d.%m.%Y %T') AS Termin_Impfung,
		...
		impfstoff.id AS Impfstoff_ID, CASE WHEN impfung.extern = 1
										   ...
										   ELSE null END AS Impfung_extern,
		CASE WHEN impfung.grundimmunisierung = 0 THEN 'FALSE' ELSE 'TRUE' END AS Grundimmunisierung,
		2 AS Impffolgenummer, TIMESTAMPDIFF(YEAR, registrierung.geburtsdatum, slot.von) AS Alter_bei_Termin
 FROM Impftermin termin
	  LEFT JOIN Impfung impfung ON termin.id = impfung.termin_id
	  LEFT JOIN Impfstoff impfstoff ON impfung.impfstoff_id = impfstoff.id
	  INNER JOIN Impfslot slot ON termin.impfslot_id = slot.id
...
(SELECT registrierung.id AS Registrierungs_ID, odi.identifier AS Ort_der_Impfung_ID, odi.name AS Ort_der_Impfung_Name,
		...
		TIMESTAMPDIFF(YEAR, registrierung.geburtsdatum, slot.von) AS Alter_bei_Termin
 ...
 WHERE dossier.krankheitIdentifier = 'COVID' and (impfung.termin_id IS NULL OR impfstoff.wHoch2Code IS NOT NULL));