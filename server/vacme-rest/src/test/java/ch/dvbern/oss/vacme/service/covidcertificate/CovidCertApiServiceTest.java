...
class CovidCertApiServiceTest {
	...
	public void setup() {
		...
		serviceUnderTest = new CovidCertApiService(
			sigHeaderBeanMock,
			covidCertSigServiceMock,
			zertServiceMock,
			jacksonMapperMock);
	...
	public void testUnvollstImpfschutz() {
		...
		final ImpfinformationDto infos = builder
			...
			.getInfos();
		try {
			VaccinationCertificateCreateDto dto = mapRegForCovidCertDto(infos.getImpfdossier(),
				infos.getImpfung1(),
				null,
				null,
	...
	public void testMappingLongName() {
		...
		VaccinationCertificateCreateDto dto =
			mapRegForCovidCertDto(dossier, impfung, null, null, CovidCertBatchType.ONLINE);
		...
		Assertions.assertEquals(
			"de Todos los Santos de Borbón y de Grecia de Silvas von und zu Hohenzollern v...",
			dto.getName().getFamilyName());
		Assertions.assertEquals(
			"Felipe Juan Pablo Alfonso Miguel Imanol Roberto Franco Carlos Martinez Maria ...",
			dto.getName().getGivenName());
	...
	public void testMapping2Doses() {
		...
		VaccinationCertificateCreateDto dto =
			mapRegForCovidCertDto(dossier, impfung, impfung2, null, CovidCertBatchType.ONLINE);
	...
	public void testMappingGenesen1PfizerOneAdditionalGrundimmunisierungPfizer() {
		...
		ImpfinformationDto infos =
			TestdataCreationUtil.createImpfinformationen(
				KrankheitIdentifier.COVID,
				reg,
				impfung,
				null,
		...
		VaccinationCertificateCreateDto dto =
			mapRegForCovidCertDto(dossier, impfung, null, List.of(impfung2), CovidCertBatchType.ONLINE);
	...
	public void testMapping1DoseJanssenOneBooster() {
		...
		Impfung impfung2 = TestdataCreationUtil.createBoosterImpfung(
	...
	public void testMappingcomplete3Impfungen() {
		...
		Impfung impfung3 = TestdataCreationUtil.createBoosterImpfung(
		...
		VaccinationCertificateCreateDto dto =
			mapRegForCovidCertDto(dossier, impfung, impfung2, List.of(impfung3), CovidCertBatchType.ONLINE);
		Assertions.assertEquals("Muster", dto.getName().getFamilyName());
		Assertions.assertEquals("Hans", dto.getName().getGivenName());
		Assertions.assertEquals("de", dto.getLanguage());
		Assertions.assertEquals(LocalDate.of(1984, 12, 12), dto.getDateOfBirth());
	...
	public void testMappingcompleteNImpfungen(int boosterCount) {
		...
		VaccinationCertificateCreateDto dto =
			mapRegForCovidCertDto(infos, infos.getImpfung1(), infos.getImpfung2(), infos.getBoosterImpfungen(),
		...
		VaccinationCertificateCreateDto dtoImpfung2 =
			mapRegForCovidCertDto(infos, infos.getImpfung1(), infos.getImpfung2(), infos.getBoosterImpfungen(),
		...
		Assertions.assertEquals(
			...
			vacc2DataDto.getMedicinalProductCode());
	...
	public void testMappingCompleteWithAddress() {
		...
		VaccinationCertificateCreateDto dto =
			mapRegForCovidCertDto(dossier, impfung, impfung2, null, CovidCertBatchType.POST);
		...
		VaccinationCertificateCreateDto dto2 =
			mapRegForCovidCertDto(dossier, impfung, impfung2, null, CovidCertBatchType.POST);
		Assertions.assertNull(dto2.getAddress());

		Adresse veryLongAddr = new Adresse();
		veryLongAddr.setAdresse1(
			"Lauchringenstr im Ort mit einem sehr langen Namen welcher dann abgeschnitten werden muss weil er sonst "
				+ "einfach zu lang ist "
				+ "und nicht mehr passen wuerde");
		veryLongAddr.setOrt(
			"Lauchringen im Ort mit einem sehr langen Namen welcher dann abgeschnitten werden muss weil er sonst "
				+ "einfach zu lang ist und nicht"
				+ " mehr passen wuerde");
		veryLongAddr.setPlz("3014");
		reg.setAdresse(veryLongAddr);
		VaccinationCertificateCreateDto dto3 =
			mapRegForCovidCertDto(dossier, impfung, impfung2, null, CovidCertBatchType.POST);
		Assertions.assertNotNull(dto3.getAddress());
		Assertions.assertEquals("BE", dto3.getAddress().getCantonCodeSender());
		Assertions.assertEquals(
			"Lauchringen im Ort mit einem sehr langen Namen welcher dann abgeschnitten werden muss weil er sonst "
				+ "einfach zu lang ist und "
				+ "nich",
			dto3.getAddress().getCity());
		Assertions.assertEquals(
			"Lauchringenstr im Ort mit einem sehr langen Namen welcher dann abgeschnitten werden muss weil er sonst "
				+ "einfach zu lang ist "
				+ "und n",
			dto3.getAddress().getStreetAndNr());
	}
	...
	public void testMappintCovidGenesen() {
		...
		VaccinationCertificateCreateDto dto =
			mapRegForCovidCertDto(infos.getImpfdossier(), impfung, null, null, CovidCertBatchType.ONLINE);
		Assertions.assertEquals("Muster", dto.getName().getFamilyName());
		Assertions.assertEquals("Hans", dto.getName().getGivenName());
		Assertions.assertEquals("de", dto.getLanguage());
		Assertions.assertEquals(LocalDate.of(1984, 12, 12), dto.getDateOfBirth());
	...
	public void testMappingFail() {
		...
		infos.getImpfdossier()
			.setStatusToAbgeschlossenOhneZweiteImpfung(infos, false, "nicht vollstaendig", LocalDate.now());
