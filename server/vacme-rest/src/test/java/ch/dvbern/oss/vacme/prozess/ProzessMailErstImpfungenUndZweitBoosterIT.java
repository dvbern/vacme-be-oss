...
public class ProzessMailErstImpfungenUndZweitBoosterIT extends AbstractProzessIT {
	...
	private final Mandant mandant = Mandant.BE;
	...
	void anzahlErstimpfungenAndZweitBoosterMail() {
		...
		long anzahlErst =
			registrierungRepo.getAnzahlErstimpfungen(mandant, OrtDerImpfungTyp.IMPFZENTRUM, zeitraumVon, zeitraumBis);
		long anzahlBooster =
			registrierungRepo.getAnzahlBoosterOrGrundimunisierungGT3Covid(
				mandant,
				OrtDerImpfungTyp.IMPFZENTRUM,
				zeitraumVon,
				zeitraumBis);
		...
		Assertions.assertEquals(
			...
			"Da alle Impfungen zwar Covid sind und als 'im Zeitraum' gelten (Erfassungdatum der Impfung) sind auch "
				+ "jeweils beide im Kalenderjahr");
		Assertions.assertEquals(
			3,
