...
class ZertifikatServiceTest {
	...
	void setUp() {
		...
		ImpfdossierService impfdossierServiceMock = Mockito.mock(ImpfdossierService.class);
	...
	void zertifikat_should_fallback_to_reading_impfstoff_by_covid_cert_prodcode_if_impfung_not_set_for_zert() {
		...
		Assertions.assertNotNull(zertifikatJax);
		Assertions.assertEquals("EU/1/20/1507", zertifikatJax.getImpfstoffJax().getCovidCertProdCode());
		Assertions.assertEquals("Moderna - Spikevax® Bivalent Original / Omicron",
			zertifikatJax.getImpfstoffJax().getDisplayName(),
			"If Impfung is not present we could potentially match the wrong Impfstoff. This should only happen as a "
				+ "fallback");

	}

	private ZertifikatJax createZertjaxAndAddToList(
		List<ZertifikatJax> list, int numDoses, LocalDate vaccDate,
