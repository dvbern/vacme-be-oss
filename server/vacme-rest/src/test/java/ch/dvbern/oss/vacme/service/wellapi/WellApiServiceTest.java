...
class WellApiServiceTest {
	...
	private final VacmeSettingsService vacmeSettingsServiceMock = Mockito.mock(VacmeSettingsService.class);
	...
	private final Impftermin pendingGalenicaTermin = TestdataCreationUtil.createImpftermin(
		...
		LocalDate.now().plusDays(7), KrankheitIdentifier.FSME);
	...
	void setUp() {
		...
		Mockito.when(vacmeSettingsServiceMock.getImpfslotCreationSettingsDTO()).thenReturn(new ImpfslotCreationSettingsDTO(15, 8, 17));
		Mockito.when(vacmeSettingsServiceMock.getTerminslotOffsetGroups()).thenReturn(3);
		Mockito.when(vacmeSettingsServiceMock.getSlotOffsetMaxTermineToDivide()).thenReturn(3);
		Mockito.when(vacmeSettingsServiceMock.isSlotOffsetDeterministicWhenLowCapacity()).thenReturn(false);
	}
	...
	@Test
	void given_first_FSME_impftermin_when_termin_in_past_then_createAppointmentInfo_must_not_sent_to_well() {
		// Testdaten vorbereiten
	...
	@Test
	void given_first_FSME_impftermin_when_termin_is_today_then_createAppointmentInfo_must_be_sent_to_well() {
		// Testdaten vorbereiten
		prepareTestdataAppointment(KrankheitIdentifier.FSME, true);
		builder.withDossiereintrag(1);
		// Dossiereintrag ermitteln
		...
		// Termin Heute soll geschickt werden wegen ad-hoc buchungen
		Impftermin terminToday = TestdataCreationUtil.createImpftermin(
			TestdataCreationUtil.createOrtDerImpfung(Kundengruppe.GALENICA),
			LocalDate.now(), KrankheitIdentifier.FSME);

		// Aufruf
		impfterminRepo.boosterTerminSpeichern(eintrag.get(), terminToday);
		// Assertions
		Mockito.verify(wellApiService, Mockito.times(1)).sendAppointmentInfoToWell(eintrag.get());
	...
	void given_first_FSME_impftermin_when_termin_in_non_galenica_odi_then_createAppointmentInfo_must_not_sent_to_well() {
		...
		Impftermin terminInNonGalenica = TestdataCreationUtil.createImpftermin(
			...
			LocalDate.now().minusMonths(2), KrankheitIdentifier.FSME);
