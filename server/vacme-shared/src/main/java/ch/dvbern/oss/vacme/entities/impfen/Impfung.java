...
@Table(
	...
	indexes = {
		@Index(name = "IX_Impfung_timestamp_impfung", columnList = "timestampImpfung, id"),
		...
		@Index(name = "IX_Impfung_sollArchiviertWerden_archiviertAm", columnList = "sollArchiviertWerden, archiviertAm, id"),
	}
)
@SqlResultSetMapping(
...
public class Impfung extends AbstractUUIDEntity<Impfung> implements ImpfInfo {
	...
	/**
	 * Dieses Flag ist normalerweise true fuer Impfung 1+2 und kann fuer nachfolgende Impfungen auch true sein, z.B.
	 * wenn eine Person mit Immunschwaeche eine dritte oder vierte Impfung braucht, um die Grundimmunisierung
	 * abzuschliessen.
	 * Spaetere Boosterimpfungen haben grundimmunisierung=false.
	 * Bei jemandem, der nur 1 Impfung brauchte, weil er Covid hatte, ist auch die zweite Impfung schon
	 * grundimmunisierung=false.
	 */
	@NotNull
	@Column(nullable = false)
	private boolean grundimmunisierung;

	// Zertifikat generieren (wichtig: alles vorher validieren, sonst fuellt es den Batchjob mit nicht erstellbaren!)
	@NotNull
	@Column(nullable = false)
	private boolean generateZertifikat = false;
	...
	@NonNull
	@Column(nullable = false)
	private boolean sollArchiviertWerden = false;
	...
	public Mandant getMandant() {
		// Die Impfung wird ueber das ODI einem Mandanten zugeordnet
		return this.getTermin().getImpfslot().getOrtDerImpfung().getMandant();
	}
}
